/*
  Created By: Christian Gonzalez (styling) Tanvi Agarwal
*/
...
import { Film, TMDBMovieListItem, SerializedFilm } from "@/types/schemas";
...
export default function ProfileFilmsSection({
  ...
  const [watchedFilms, setWatchedFilms] = useState<SerializedFilm[]>([]);
  ...
  const fetchFilms = async () => {
    ...
    try {
      const res = await fetch(
        `/api/watched?username=${username}${
          showLikedOnly ? "&liked=true" : ""
        }`,
        { cache: "no-store" },
      );
      ...
      <div className="mt-8">
        {loadingFilms ? (
          ...
          <div className="text-center text-[#89a] text-sm italic">
            {showLikedOnly
              ? "No films liked so far."
              : "No films watched so far."}
          </div>
        ) : (
