...
import type { TMDBCastMember, TMDBCrewMember } from "@/types/schemas";
...
export default async function MoviePage({ params, searchParams }: MoviePageProps) {
  ...
  // URL State helpers
  const sectionParam = typeof resolvedSearchParams === "object" && resolvedSearchParams ? resolvedSearchParams["section"] : undefined;
  const showAllCastParam = typeof resolvedSearchParams === "object" && resolvedSearchParams ? resolvedSearchParams["showAllCast"] : undefined;
  const showAllCrewParam = typeof resolvedSearchParams === "object" && resolvedSearchParams ? resolvedSearchParams["showAllCrew"] : undefined;

  const showAllCast = (Array.isArray(showAllCastParam) ? showAllCastParam[0] : showAllCastParam) === "1"; // check if user expanded list
  const showAllCrew = (Array.isArray(showAllCrewParam) ? showAllCrewParam[0] : showAllCrewParam) === "1";
  
  const sectionRaw = Array.isArray(sectionParam) ? sectionParam[0] : sectionParam;
  const allowedSections = ["cast", "crew", "details", "genres", "links"] as const; //  tabs
  const activeSection = (sectionRaw || "cast").toLowerCase();
  const currentSection = allowedSections.includes(activeSection as any) ? activeSection : "cast"; // fallback if url doesnt work

  // Data fetching
  const movie = await getMovieDetails(id); // fetch on server
  
  const releaseYear = movie.release_date ? movie.release_date.split("-")[0] : null; // grab year 
        ...
        <div className="flex flex-1 flex-col pt-4 md:pt-12 text-slate-100">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-1 text-amber-400">
              <span className="text-lg font-bold text-white mr-1">{movie.vote_average.toFixed(1)}</span>
            ...
            {movie.genres?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {movie.genres.map((genre) => (
                  <span key={genre.id} className="text-xs font-medium text-slate-300 bg-slate-800 px-2 py-1 rounded">
                    {genre.name}
                  </span>
                ))}
              </div>
        ...
        <div className="min-h-[200px]">
          {currentSection === "cast" && (
            <div>
              ...
              {cast.length > 20 && (
                <div className="mt-4">
                  <Link
                    href={showAllCast ? `/movies/${id}?section=cast#sections` : `/movies/${id}?section=cast&showAllCast=1#sections`}
          ...
          {currentSection === "crew" && (
            <div>
              ...
              {crew.length > 20 && (
                <div className="mt-4">
                  <Link
                    href={showAllCrew ? `/movies/${id}?section=crew#sections` : `/movies/${id}?section=crew&showAllCrew=1#sections`}
                    className="text-xs font-medium text-emerald-400 hover:underline"
                  >
          ...
          {currentSection === "details" && (
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Languages</h3>
                <p className="text-sm text-slate-300">
                  {movie.spoken_languages?.map(l => l.english_name || l.name).join(", ") || "N/A"}
                </p>
              </div>
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Studios</h3>
                <p className="text-sm text-slate-300">
                  {studios.map(s => s.name).join(", ") || "N/A"}
                </p>
              </div>
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Countries</h3>
                <p className="text-sm text-slate-300">
                  {movie.production_countries?.map(c => c.name).join(", ") || "N/A"}
                </p>
              </div>
              <div>
                 <h3 className="mb-2 text-xs font-semibold uppercase text-slate-500">Status</h3>
          ...
          {currentSection === "links" && (
             <div className="flex gap-4">
                {movie.homepage && (
                  <a href={movie.homepage} target="_blank" className="text-sm text-emerald-400 hover:underline">
                    Official Website ↗
                  </a>
                )}
                {movie.imdb_id && (
                  <a href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank" className="text-sm text-emerald-400 hover:underline">
                    IMDb Page ↗
                  </a>
                )}
             </div>
